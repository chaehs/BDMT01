<template>
  <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-4 flex flex-col h-full hover:shadow-2xl transition-all duration-300 relative overflow-hidden group cursor-pointer border-transparent hover:border-blue-200">
    ...
    <div class="mb-3 relative z-10 flex justify-between items-start">
      ...
      <div class="flex items-center bg-yellow-400 px-2.5 py-1.5 rounded-xl shadow-sm border border-yellow-500/20">
        ...
        <span class="text-xs font-black text-white">{{ racket.rating || 0 }}</span>
        <span class="text-[10px] text-yellow-100/80 ml-1 font-bold">({{ racket.review_count || 0 }})</span>
      </div>
    </div>
    ...
    <div class="flex gap-4 mb-4 relative z-10 flex-1 min-h-0">
      <!-- Image Area -->
      <div class="w-[35%] aspect-[3/4] bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden border border-gray-200 shadow-inner flex items-center justify-center p-2 relative group-hover:scale-[1.05] transition-transform duration-500">
...
const racketImageUrl = computed(() => {
  ...
  // Supabase Image Transformation (Optional: width=300 for card)
